...
async function parseArguments(): Promise<CliArgs> {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('disable-model-fallback', {
      type: 'boolean',
      description:
        'Disable automatic fallback from gemini-2.5-pro to gemini-2.5-flash on rate limits',
      default:
        process.env.GEMINI_DISABLE_MODEL_FALLBACK === 'false' ? false : true,
    })
...
export async function loadCliConfig(
  ...
  return new Config({
    ...
    disableModelFallback:
      argv['disable-model-fallback'] ?? settings.disableModelFallback ?? true,
    retryDelayMultiplier:
      argv['retry-delay-multiplier'] ?? settings.retryDelayMultiplier ?? 2,
